...
def _impersonate_lsass():
    ...
    try:
        ...
        impersonation_token = lsass_token.duplicate(
            type=gdef.TokenImpersonation, impersonation_level=gdef.SecurityImpersonation
        )
...
def _normalize_genarate_paths_chromium(
    paths: Union[str, list], channel: Union[str, list] = None
):
    channel = channel or [""]
...
class _LinuxPasswordManager:
    ...
    def __get_secretstorage_item_dbus(self, schema: str, application: str):
        with contextlib.closing(dbus.SessionBus()) as connection:
            ...
            _, _, secret, _ = secret_service.GetSecrets([object_path], session)[
                object_path
            ]
            return bytes(secret)
    ...
    def __get_secretstorage_item_jeepney(self, schema, application):
        ...
        with _JeepneyConnection(*args) as connection:
            object_path = connection.call_method(
                "SearchItems",
                "a{ss}",
                {"xdg:schema": schema, "application": application},
            )
            object_path = list(filter(lambda x: len(x), object_path))
            if len(object_path) == 0:
                raise RuntimeError(f"Can not find secret for {application}")
            ...
            _, _, secret, _ = connection.call_method(
                "GetSecrets", "aoo", [object_path], session
            )[object_path]
            return secret
    ...
    def __get_kdewallet_password_jeepney(self, os_crypt_name):
        folder = f"{os_crypt_name.capitalize()} Keys"
        key = f"{os_crypt_name.capitalize()} Safe Storage"
        with _JeepneyConnection(
            "/modules/kwalletd5", "org.kde.kwalletd5", "org.kde.KWallet"
        ) as connection:
...
class ChromiumBased:
    ...
    def __init__(
        self, browser: str, cookie_file=None, domain_name="", key_file=None, **kwargs
    ):
    ...
    def __add_key_and_cookie_file(
        ...
        if os_name == "osx":
            ...
            iterations = 1
            self.v10_key = PBKDF2(
                CHROMIUM_DEFAULT_PASSWORD, self.salt, self.length, iterations
            )
            ...
            if key_file:
                with open(key_file, "rb") as f:
                    ...
                    if "app_bound_encrypted_key" in key_file_json["os_crypt"]:
                        try:
                            app_bound_encrypted_key = key_file_json["os_crypt"][
                            ...
                            if (
                                ...
                                key_blob_encrypted = binascii.a2b_base64(
                                    app_bound_encrypted_key
                                )[4:]
                                ...
                                if windows and _is_admin():
                                    try:
                                        with _impersonate_lsass():
                                            key_blob_system_decrypted = (
                                                windows.crypto.dpapi.unprotect(
                                                    key_blob_encrypted
                                                )
                                            )

                                        # Decrypt with user DPAPI
                                        key_blob_user_decrypted = (
                                            windows.crypto.dpapi.unprotect(
                                                key_blob_system_decrypted
                                            )
                                        )
                                        ...
                                        parsed_data = _parse_key_blob(
                                            key_blob_user_decrypted
                                        )
                                        self.v20_key = _derive_v20_master_key(
                                            parsed_data
                                        )
                                    except Exception:
    ...
    def load(self):
        ...
        with _DatabaseConnetion(self.cookie_file) as con:
            ...
            has_integrity_check_for_cookie_domain = (
                self._has_integrity_check_for_cookie_domain(cur)
            )
            try:
                # chrome <=55
                cur.execute(
                ...
                try:
                    ...
                    if e.args[0].startswith(
                        ...
                        raise BrowserCookieError(
                            "File {} is not a Chromium-based browser cookie file".format(
                                self.tmp_cookie_file
                            )
                        )
            ...
            for item in cur.fetchall():
                ...
                if expires_nt_time_epoch == 0:
                    ...
                    expires = (
                        expires_nt_time_epoch / 1000000
                    ) - self.UNIX_TO_NT_EPOCH_OFFSET

                value = self._decrypt(
                    value, enc_value, has_integrity_check_for_cookie_domain
                )
    ...
    def _decrypt(
        self, value, encrypted_value, has_integrity_check_for_cookie_domain=False
    ):
        ...
        if sys.platform == "win32":
            ...
            if encrypted_value[:3] == b"v20":
                if hasattr(self, "v20_key") and self.v20_key:
                    try:
                        return self._decrypt_v20_cookie(
                            encrypted_value, has_integrity_check_for_cookie_domain
                        )
                    except Exception:
        ...
        if not hasattr(self, "v11_key"):
            assert encrypted_value[:3] != b"v11", (
                "v11 keys should only appear on Linux."
            )
        keys = (
            (self.v11_key, self.v11_empty_key)
    ...
    def _decrypt_v20_cookie(
        self, encrypted_value, has_integrity_check_for_cookie_domain=False
    ):
        ...
        decrypted_cookie = cookie_cipher.decrypt_and_verify(
            encrypted_cookie, cookie_tag
        )
...
class OperaGX(ChromiumBased):
    ...
    def __init__(self, cookie_file=None, domain_name="", key_file=None):
        args = {
            ...
            "windows_keys": _genarate_win_paths_chromium(
                "Opera Software\\Opera GX {channel}\\Local State", channel=["Stable"]
            ),
            "os_crypt_name": "chromium",
            "osx_key_service": "Opera Safe Storage",
            "osx_key_user": "Opera",
...
class FirefoxBased:
    ...
    def __init__(
        self, browser_name, cookie_file=None, domain_name="", key_file=None, **kwargs
    ):
        ...
        self.session_file_lz4 = os.path.join(
            os.path.dirname(self.cookie_file),
    ...
    def __expand_and_check_path(
        self, paths: Union[str, List[str], Dict[str, str], List[Dict[str, str]]]
    ) -> str:
    ...
    def __find_cookie_file(
        self, linux_data_dirs=None, windows_data_dirs=None, osx_data_dirs=None
    ):
        cookie_files = []
        ...
        cookie_files = (
            glob.glob(
                os.path.join(
                    FirefoxBased.get_default_profile(user_data_path), "cookies.sqlite"
                )
            )
            or cookie_files
        )
        ...
        try:
            ...
            for window in json_data.get("windows", []):
                for cookie in window.get("cookies", []):
                    if self.domain_name == "" or self.domain_name in cookie.get(
                        "host", ""
                    ):
        ...
        with _DatabaseConnetion(self.cookie_file, True) as con:
            ...
            try:
                ...
                if e.args[0].startswith(("no such table: ", "file is not a database")):
                    raise BrowserCookieError(
                        "File {} is not a Firefox cookie file".format(
                            self.tmp_cookie_file
                        )
                    )
                raise
...
class LibreWolf(FirefoxBased):
    ...
    def __init__(self, cookie_file=None, domain_name="", key_file=None):
        args = {
            ...
            "osx_data_dirs": ["~/Library/Application Support/librewolf"],
        }
...
class Safari:
    ...
    def __parse_cookie(self, page: BytesIO, cookie_offset: int):
        ...
        expiry_date = int(
            struct.unpack("<d", page.read(8))[0] + self.APPLE_TO_UNIX_TIME
        ...
        return create_cookie(
            host, path, is_secure, expiry_date, name, value, is_httponly
        )
            ...
            for line in f.read().splitlines():
                ...
                if domain.find(self.domain_name) >= 0:
                    cookie = create_cookie(
                        domain, path, secure, expires, name, value, False
                    )
                    cj.set_cookie(cookie)
        return cj
...
__all__ = ["BrowserCookieError", "load", "all_browsers", "BROWSERS"] + [
    i.__name__ for i in all_browsers
]
